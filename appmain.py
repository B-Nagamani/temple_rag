...
With Guard Rails, Conversation History, and Enhanced UX
"""
...
def initialize_session_state():
    ...
    if 'guard_rail_blocks' not in st.session_state:
        st.session_state.guard_rail_blocks = 0
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        with col2:
            st.markdown(f"""
                <div class='stats-box'>
                    <div class='stat-number'>{st.session_state.guard_rail_blocks}</div>
        ...
        st.markdown("""
            <div style='background: rgba(255,255,255,0.2); padding: 15px; border-radius: 10px;'>
                ...
                <div style='text-align: center;'>
                    ...
                    <span class='feature-badge'>Semantic</span>
        ...
        # History button
        st.markdown("---")
        if st.button("📜 View Full History", use_container_width=True, key="history_btn"):
    ...
    if rag.is_database_empty():
        ...
        st.markdown(f"""
            <div class='warning-box'>
                ...
                Using semantic sentence chunking with guard rails...<br>
                This will take 20-40 minutes depending on PDF count.<br>
...
def render_history_panel():
    ...
    history = st.session_state.rag_system.conversation_history.get_history()
    ...
    for i, entry in enumerate(reversed(history), 1):
        ...
        if entry.get('sources'):
            sources_text = f"<br><small>📚 Sources: {', '.join(entry['sources'][:2])}</small>"
        
        st.markdown(f"""
            <div class='history-item'>
                <strong>Q{len(history)-i+1}:</strong> {entry['question']}<br>
...
def render_chat_interface():
    ...
    st.markdown("<div class='subtitle'>🛡️ Guard Rails • 📜 History • 🧠 Semantic RAG</div>", 
               unsafe_allow_html=True)
    st.markdown("---")
        ...
        try:
            ...
            st.markdown("""
                <div class='info-box'>
                    ...
                    ✅ <strong>Guard Rails:</strong> Only temple-related questions allowed<br>
                    ✅ <strong>Conversation History:</strong> Full tracking of Q&A pairs<br>
                    ✅ <strong>Semantic Chunking:</strong> 800 chars with overlap<br>
                    ✅ <strong>Response Sanitization:</strong> Clean, safe outputs
                </div>
            """, unsafe_allow_html=True)
    ...
    if not st.session_state.messages:
        ...
        st.markdown("""
            <div class='guard-rail-box'>
                <strong>🛡️ Guard Rails Notice:</strong><br>
                I can only answer questions about Hindu temples, Jyotirlingas, and Shakti Peethas.
                Questions about politics, entertainment, stocks, weather, etc. will be blocked.
            </div>
        """, unsafe_allow_html=True)
            ...
            if message["role"] == "user":
                ...
                st.markdown(f"""
                    <div class='error-message'>
                        <strong>🛡️ Guard Rail:</strong><br>{message["content"]}
                    </div>
                """, unsafe_allow_html=True)
            else:
                st.markdown(f"""
    ...
    if ask_button and user_question:
        ...
        with st.spinner("🔮 Processing with guard rails..."):
            try:
                answer = st.session_state.rag_system.answer_question(user_question)
                
                # Check if guard rail blocked
                if answer.startswith("❌") or answer.startswith("⚠️"):
                    st.session_state.messages.append({
                        "role": "error",
                        "content": answer
                    })
                    st.session_state.guard_rail_blocks += 1
                else:
                    if not answer or len(answer.strip()) < 10:
                        answer = "• I apologize, but I couldn't generate a proper answer based on the PDF documents."
    ...
    suggestions = [
        ...
        ("🌺 18 Shakti Peethas", "Tell me about 18 Maha Shakti Peethas"),
    ...
    more_suggestions = [
        ...
        ("📜 My History", "Show my conversation history"),
        ("🛡️ Test Guards", "Tell me about politics")
    ]
...
def main():
    ...
    st.markdown("""
        <div style='text-align: center; color: #666; font-size: 12px; padding: 20px;'>
            <p>🕉️ Temple Guider AI - Protected Edition</p>
            <p>🛡️ Guard Rails Active • 📜 Full History Tracking • 🧠 Semantic Chunking • 🔒 Safe Responses</p>
